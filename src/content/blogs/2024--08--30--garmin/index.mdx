...
Note that, due to Astro's technical limitation, we prefer to render and load the data on client-side like below.
Meaning the
:
...
import {
	...
	PaceViewer,
	CadenceViewer
} from '../../../components/fit.jsx'

<FITDataProvider id="my-activity" client:only="react" fitFile={activity_link}>

    <a href={`${activity_link}`}>Downloadable link to the FIT data, built by webpack</a>

</FITDataProvider>

<HeartRateViewer client:only='react' providerID='my-activity' />
<PaceViewer client:only='react' providerID='my-activity' />
<CadenceViewer client:only='react' providerID='my-activity' />
```
